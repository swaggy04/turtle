...
export function FileExplorer() {
  ...
  return (
    <aside className="flex h-full min-h-0 w-64 shrink-0 flex-col border-r border-white/10 bg-[#0a0a0a]">
      {/* Header */}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {files.length === 0 ? (
          <div className="px-2 py-4">
            <p className="text-xs text-white/30">No files generated yet.</p>
          </div>
        ) : (
          <div className="space-y-0.5">
            {files.map((file) => {
              const isActive = state.activeFile === file.path;
